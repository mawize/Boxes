                ...
                case "define":
                    {
                        if (args.Parameters.Count > 1)
                            if (!args.Player.TempPoints.Any(p => p == Point.Zero))
                            {
								...
								if(args.Player.Group.HasPermission("boxes.admin") || BoxesPlugin.Config.MaxTilesPerUser==0 || ((width * height)+BoxMan.GetUsersBoxedTiles(args.Player.Name)) <= BoxesPlugin.Config.MaxTilesPerUser)
